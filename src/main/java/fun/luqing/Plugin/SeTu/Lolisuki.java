...
public class Lolisuki {

    public Lolisuki(GroupMessage groupMessage) {
    ...
    private JSONObject sendApiRequest(Map<String, Object> customParams) {
        try {
            String apiUrl = "https://lolisuki.cn/api/setu/v1";
    ...
    private void handleApiResponse(JSONObject resp, GroupMessage groupMessage) {
        ...
        if (resp.getInt("code") != 0 || resp.getJSONArray("data").isEmpty()) {
        ...
        for (int i = 0; i < data.length(); i++) {
            ...
            try {
                JSONArray fullUrls = setu.getJSONArray("fullUrls");
                String imageUrl = fullUrls.getJSONObject(0).getString("regular");
